 ...
 * Copyright (C) TODO
 *
 * camera helper for imx520 sensor
 * 
...
class CamHelperImx520 : public CamHelper
{
public:
	CamHelperImx520();
...
private:
	 ...
	 * in units of lines.
	 */
	static constexpr int frameIntegrationDiff = 10;
...
CamHelperImx520::CamHelperImx520()
	: CamHelper(std::make_unique<MdParserSmia>(registerList), frameIntegrationDiff)
{
}

uint32_t CamHelperImx520::gainCode(double gain) const
{
	return static_cast<uint32_t>(1024 - 1024 / gain);
}

double CamHelperImx520::gain(uint32_t gainCode) const
{
	return 1024.0 / (1024 - gainCode);
}

void CamHelperImx520::prepare(libcamera::Span<const uint8_t> buffer, Metadata &metadata)
{
...
std::pair<uint32_t, uint32_t> CamHelperImx520::getBlanking(Duration &exposure,
							   ...
							   Duration maxFrameDuration) const
{
	...
	if (shift) {
		...
		frameLength <<= shift;
		exposureLines = CamHelperImx520::exposureLines(exposure, lineLength);
		exposureLines = std::min(exposureLines, frameLength - frameIntegrationDiff);
		exposure = CamHelperImx520::exposure(exposureLines, lineLength);
	}
...
{
	return new CamHelperImx520();
}

static RegisterCamHelper reg("imx520", &create);
